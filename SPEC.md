...
`syncHub.js` 提供独立的原生 WebSocket 通道 `/sync/ws`：任一标签页通过写接口增删改待办、分类、设置或笔记后，服务端向所有已连接的前端页面广播 `{ "type": "changed" }`，页面收到后调用 `loadData()` 全量重拉并重渲染，实现多标签页秒级同步，无需手动刷新。

- **广播触发**：仅在所有数据写接口成功分支调用（创建/更新/删除分类、分类排序、创建/更新/删除待办、保存设置、写入笔记）；GET 查询不触发，避免广播风暴与死循环。
- **承载信号**：广播只携带「数据已变更」标记，业务数据始终由下游重拉，服务端不缓存业务数据。
- **断线重连**：前端约 2 秒自动重连；每 30 秒 ping 保活。
- **鉴权**：与 `/api/*` 数据接口保持一致，本模块不做鉴权（局域网个人应用）。
...
- `WS /sync/ws`：数据变更广播 WebSocket 通道（收到 `changed` 后前端重拉数据）
...
├── index.html      # 工作台式前端：导航 + 待办页 + 运行日志页 + 实时同步
